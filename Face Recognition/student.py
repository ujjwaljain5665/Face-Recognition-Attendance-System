...
class student:
    def __init__(self,window):
        ...
        img=Image.open("Images/Student/pic1.jpg")
        img=img.resize((300,200),Image.ANTIALIAS)
        self.img=ImageTk.PhotoImage(img)
        imglbl=Label(self.f1,image=self.img).place(x=60,y=0,width=300,height=200)
        # top frame -> Heading
        heading=Label(self.f1,text="Add Students Details",font=("times new roman",40,"bold"),bg="white",fg="darkgreen").place(x=550,y=40)

        # main frame
        self.f2=Frame(self.window,bg="white")
        self.f2.place(x=10,y=190,width=1260,height=450)
        ...
        self.left.place(x=15,y=5,width=600,height=430)
        ...
        Takephtsample=Button(self.left,text="Take Photo Sample",font=("times new roman",12,"bold"),bg="darkgreen",fg="white",relief=RIDGE)
        Takephtsample.place(x=110,y=370,width=150,height=30)
        ...
        self.right.place(x=635,y=5,width=610,height=430)

        # search label
        search=Label(self.right,text="Search By",font=("times new roman",12,"bold"),bg="lightblue",fg="black")
        search.place(x=10,y=10)
        search_cmb_bx=ttk.Combobox(self.right,font=("times new roman",12),state='readonly',justify=CENTER)
        search_cmb_bx['values']=("Select","Roll No","Phone Number","Name","Id")
        search_cmb_bx.place(x=100,y=10,width=100)
        search_cmb_bx.current(0)
        srch_entry=Entry(self.right,font=("times new roman",15),bg="lightgrey")
        srch_entry.place(x=220,y=12,width=150)

        # searchbtn
        srch=Button(self.right,text="Search",font=("times new roman",12,"bold"),bg="green",fg="white",relief=RIDGE)
        srch.place(x=380,y=10,width=100,height=30)
        # showall
        shwal=Button(self.right,text="Show All",font=("times new roman",12,"bold"),bg="green",fg="white",relief=RIDGE)
    ...
    def add_data(self):
        ...
        if self.course_cmb_bx.get()=="" or self.dep_cmb_bx.get()=="" or self.fname.get()=="" or self.lname.get()=="" or self.email_id.get()=="" or self.univ_roll.get()=="" or self.studid.get()=="" or self.gender_cmb_bx.get()=="" or self.year_bx=="" or self.section=="" or self.sem_bx=="" or self.phone=="" or self.address=="":
            ...
            if self.rad.get()==1:
                ...
                self.var="No"
            try:
                con=pymysql.connect(host="localhost",user="root",password="",database="facerecognition")
                cur=con.cursor()
